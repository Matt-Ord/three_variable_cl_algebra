...
x_expr = (a_expr + a_dagger_expr) / sp.sqrt(2)
p_expr = -sp.I * hbar * (a_dagger_expr - a_expr) / sp.sqrt(2)
...
@cache
@timed
def get_system_deriviative(ty: Literal["zeta", "alpha", "phi"]) -> sp.Expr:
    linear_term = get_linear_term()
    kinetic_term = get_kinetic_term()
    harmonic_term = get_harmonic_term()
    expr = linear_term + kinetic_term + harmonic_term

    return sp.factor(extract_action(action_from_expr(expr), ty))
...
def get_environment_derivative(ty: Literal["zeta", "alpha", "phi"]) -> sp.Expr:
    ...
    return shift_derivative + environment_derivative
