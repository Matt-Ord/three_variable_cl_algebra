...
def plot_uncertainty_x_against_m_w() -> None:
    ...
    eta_lambda = np.ones_like(eta_m_grid)
    uncertainty = (
        evaluate_equilibrium_uncertainty_x(eta_m_grid, eta_omega_grid, eta_lambda)
        * eta_m_grid
    )
...
def plot_uncertainty_x_against_lambda_m() -> None:
    ...
    uncertainty = (
        evaluate_equilibrium_uncertainty_x(eta_m_grid, eta_omega, eta_lambda_grid)
        * eta_m_grid
    )
...
def plot_uncertainty_x_against_lambda_free_particle() -> None:
    ...
    eta_m = 1 * np.ones_like(eta_lambda)
    uncertainty = evaluate_equilibrium_uncertainty_x_free(eta_m, eta_lambda) * eta_m
    ...
    ax.set_title(r"Plot of uncertainty in x against $\eta_\lambda$")
    ax.set_xlabel(r"$\eta_\lambda$")
    ax.set_ylabel(r"Uncertainty in x * $\eta_m$")
    fig.show()
